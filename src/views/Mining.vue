<template>
  <div class="mining">
    ...
    <TestTable :product="product" :edit="edit" v-if="!isEdit" category="/mining/" category-header="MINING" />
  </div>
</template>
...
export default {
  ...
  async mounted () {
    this.product = await ProductService.getProduct('mining')
    await ProductService.getFile(this.product, 'mining')
  },
  methods: {
    edit (item) {
    ...
    async writeProductData() {
      ...
      if (this.brand !== '' && this.code !== '' && this.selectedFile !== '') {
        await ProductService.writeProduct('mining', this.code, this.brand, this.url, this.selectedFile)
      } else {
